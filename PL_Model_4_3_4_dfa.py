...
from models import model434dfa
import torchmetrics
...
class pl_model(pl.LightningModule):
    def __init__(self, net_param, train_param, log_path=None):
        ...
        self.KldLoss = nn.KLDivLoss()                                                               # kld 损失
        self.ReconstructLoss = nn.MSELoss(reduction="mean")
    ...
    def training_step(self, batch, batch_idx):
        ...
        encoder_outputs, encoder_hiddens, feature_src, reconstruct_src = self.net(data_src, data_length_src)
        ...
        _, _, feature_trg, reconstruct_trg = self.net(data_trg, data_length_trg)


        # Kld loss
        zn_src = Variable(torch.Tensor(np.random.normal(0,1, feature_src.shape)).float())
        zn_trg = Variable(torch.Tensor(np.random.normal(0,1, feature_trg.shape)).float())
        kld_src = self.KldLoss(F.log_softmax(feature_src), F.softmax(zn_src))
        kld_trg = self.KldLoss(F.log_softmax(feature_trg), F.softmax(zn_trg))

        # reconstruct loss
        # bs, ts, cn, h, w = data_src.shape
        # bt, tt, _, _, _ = data_trg.shape
        # data_src_ = data_src.contiguous().view(bs*ts, cn, h, w) 
        # reconstruct_loss_s = self.ReconstructLoss(data_src_, reconstruct_src)
        # data_trg_ = data_trg.contiguous().view(bt*tt, cn, h, w)
        # reconstruct_loss_t = self.ReconstructLoss(data_trg_, reconstruct_trg)

        # ctc loss
        ctcloss = self.CTCLoss(encoder_outputs, label_src , data_length_src, label_length_src)
        
        totalloss = ctcloss + kld_src + kld_trg

        self.log("total_loss", totalloss)
        self.log("ctc_loss", ctcloss)
        self.log("dfa_loss_s", kld_src)
        self.log("dfa_loss_t", kld_trg)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outputs, hiddens, _, _ = self.net(source_inputs, source_lengths)
