...
def beam_search(mat: np.ndarray, chars: str, beam_width: int = 25, lm: Optional[LanguageModel] = None, lexicon=[], top=5) -> str:
    """Beam search decoder.
    ...
    # sort by probability
    labelings = last.sort_labelings()  # get most probable labeling
    # print(len(labelings))
    # exit(0)
    charlabelsreal = []
    charlabelswrong = []
    for label in labelings:
        res = ''.join([chars[l] for l in label])
        if len(lexicon) != 0: 
            if res in lexicon:
                charlabelsreal.append(res)
            else:
                charlabelswrong.append(res)
        else:
            charlabelswrong.append(res)
    
    charlabels = charlabelsreal + charlabelswrong
    return charlabels[:top]
