# ====================================================== #
#  this version only have encoder part, but not decoder  #
# ====================================================== #
import os
...
class pl_model(pl.LightningModule):
    def __init__(self, net_param, train_param):
        ...
        # 创建模型
        self.net = lenet_lstm.Lenet_lstm(
            self.net_param["embed_input_nc"], 
            self.net_param["embed_output_size"], 
            self.net_param["embed_input_shape"], 
            self.net_param["num_layer"], 
            self.net_param["hidden_size"], 
            self.net_param["directions"], 
            self.net_param["encoder_output_size"]
        )
        # 生成损失函数                       
    ...
    def forward(self, x, x_lengths):
        y = self.net(x, x_lengths)  # 【b, ts, cln】
        return y
    ...
    def validation_epoch_end(self, outputs):
        ...
        with open(os.path.join(os.getcwd(), self.train_param["save_result"]), "a+") as f:
            ...
            for i in range(len(pres)):
                f.write(pres[i] + " " + labels[i] + "\n")
            
        f.close()

        self.metric.reset()
        char_error_rate = self.metric(pres, labels)
